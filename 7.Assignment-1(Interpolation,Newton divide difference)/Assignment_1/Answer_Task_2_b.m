...
fprintf('\n')
%Error Bound finding
%Assume that,gx = (x-0) (x-.25) (x - 0.5) (x-1)ID:1902008
...
%fprintf('The maximum absolute value on [0,1] of the
% polynomial g(x) is: %5.6f\n',max_val_gx) 
...
%fprintf('The absolute maximum value of 
%fourth derivative of f(x) is : %5.4f\n',max_fx)
fact = factorial( n) ;
error_bound = (max_val_gx * max_fx./fact);
fprintf('The error bound is : %5.6f\n',error_bound)
%Plot function,f(x)=cos(x)+sin(x) and 
%Polynomial,P(x)= -(0.0793*x^3)-(0.5455*x^2)+(1.0066*x^1)+(1.00) 
...
polynomial = -(0.0793*x_nodes.^3)-(0.5455*x_nodes.^2)...
 +(1.0066*x_nodes.^1)+(1.00);
